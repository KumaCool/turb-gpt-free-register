...
CLOAK_BROWSER_VERSION = ""      # 空=latest（启动传入探测到的最新号，不回落捆绑 146）；钉死填完整号，如 151.0.7922.108.6
...
- WebUI「本地指纹浏览器」分组可用下拉选择 `CLOAK_BROWSER_VERSION`（`latest (当前号)` 或完整内核号）和 `CLOAK_RELEASE_CHANNEL`（stable/preview）。选 latest 时启动传入探测到的最新完整号，不把包内捆绑 146 当成 latest。免费号必须带当前平台包才进下拉；没有 `linux-arm64` 包的版本（例如 `146.0.7680.177.5`）不会再标成可跑。已保存但不存在的钉仍保留，文案为「不可用」。有 License 时只把 Pro 号当可跑项。
